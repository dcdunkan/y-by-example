...
// First, let's import the main <code>Bot</code> class from grammY.
...
// You can listen for any type of messages using the <code>"message"</code> filter query.
bot.on("message", (ctx) => ctx.reply("Hello, World!"));

// The method <code class="font-mono">reply</code> is used for replying to that incoming message.
